...
class DatabaseSchemaGrapher:
    ...
    - Create dependency matrix plots
    ...
    def create_dependency_matrix_plot(self, schema_name: str = None, max_objects: int = 50) -> go.Figure:
        """
        Create a matrix dependency plot showing relationships between database objects.

        Args:
            schema_name: Optional schema to filter by
            max_objects: Maximum number of objects to include in matrix (for readability)

        Returns:
            Plotly Figure object containing the matrix plot
        """
        # Filter tables by schema if specified
        filtered_tables = self.tables
        filtered_relationships = self.relationships
        ...
        # Limit the number of objects for readability
        if len(filtered_tables) > max_objects:
            ...
            # Sort tables by connection count and take top N
            sorted_tables = sorted(filtered_tables,
                                   key=lambda t: table_connections.get(t['full_name'], 0),
                                   reverse=True)
            filtered_tables = sorted_tables[:max_objects]

            # Filter relationships to only include tables in our subset
            table_names = {t['full_name'] for t in filtered_tables}
        ...
        object_names = [t['full_name'] for t in filtered_tables]
        object_display_names = [t['name'] for t in filtered_tables]  # Just table name for display
        n_objects = len(object_names)

        if n_objects == 0:
            fig = go.Figure()
            fig.add_annotation(text="No objects to display", x=0.5, y=0.5, showarrow=False)
            return fig

        # Create dependency matrix
        dependency_matrix = np.zeros((n_objects, n_objects))

        # Populate matrix based on relationships
        name_to_index = {name: i for i, name in enumerate(object_names)}

        for rel in filtered_relationships:
            from_idx = name_to_index.get(rel['from_table'])
            to_idx = name_to_index.get(rel['to_table'])

            if from_idx is not None and to_idx is not None:
                dependency_matrix[from_idx][to_idx] = 1  # from_table depends on to_table

        # Create the heatmap
        fig = go.Figure(data=go.Heatmap(
            z=dependency_matrix,
            x=object_display_names,
            y=object_display_names,
            colorscale=[
                [0, 'white'],      # No dependency
                [1, 'darkblue']    # Has dependency
            ],
            showscale=True,
            colorbar=dict(
                title="Dependency",
        ...
        # Update layout
        title_text = f'Database Dependency Matrix - {schema_name or "All Schemas"}'
        if len(filtered_tables) == max_objects and schema_name is None:
            title_text += f' (Top {max_objects} Most Connected)'
        elif len(filtered_tables) == max_objects:
            title_text += f' (Top {max_objects} Most Connected)'

        fig.update_layout(
            title=title_text,
            xaxis_title="Referenced Object (Dependency Target)",
            yaxis_title="Referencing Object (Depends On)",
            width=max(600, n_objects * 25),
            height=max(600, n_objects * 25),
            xaxis=dict(
                tickangle=45,
                side='bottom'
            ),
            yaxis=dict(
                tickangle=0,
                autorange='reversed'  # Put first item at top
            )
        )

        return fig

    def export_dependency_matrix_plot(self, schema_name: str = None, max_objects: int = 50, output_path: str = None) -> str:
        """
        Export dependency matrix plot as PNG using matplotlib.
        
        Args:
            schema_name: Optional schema to filter by
            max_objects: Maximum number of objects to include in matrix
        ...
        if output_path is None:
            ...
            output_path = f"dependency_matrix{schema_suffix}_{timestamp}.png"
        ...
        if not filtered_tables:
            ...
            ax.set_title(f'Database Dependency Matrix - {schema_name or "All Schemas"}')
            ax.axis('off')
            plt.tight_layout()
            ...
            print(f"Dependency matrix exported to: {abs_path}")
            return abs_path
        ...
        if n_objects == 0:
            # Create an empty plot
            fig, ax = plt.subplots(figsize=(10, 8))
            ax.text(0.5, 0.5, 'No objects to display',
                    ha='center', va='center', transform=ax.transAxes, fontsize=16)
            ax.set_title('Database Dependency Matrix')
            ax.axis('off')
            plt.tight_layout()
            plt.savefig(output_path, dpi=150, bbox_inches='tight')
            plt.close()
        ...
        # Create the plot
        fig_size = max(8, n_objects * 0.4)
        fig, ax = plt.subplots(figsize=(fig_size, fig_size))
        
        # Create heatmap
        im = ax.imshow(dependency_matrix, cmap='Blues', aspect='equal')
        
        # Set ticks and labels
        ax.set_xticks(range(n_objects))
        ax.set_yticks(range(n_objects))
        ax.set_xticklabels(object_display_names, rotation=45, ha='right')
        ax.set_yticklabels(object_display_names)
        
        # Add labels
        ax.set_xlabel('Referenced Object (Dependency Target)')
        ax.set_ylabel('Referencing Object (Depends On)')
        ...
        cbar.set_label('Dependency', rotation=270, labelpad=15)
        cbar.set_ticks([0, 1])
        cbar.set_ticklabels(['No', 'Yes'])
        
        # Add grid
        ax.set_xticks([x - 0.5 for x in range(1, n_objects)], minor=True)
        ax.set_yticks([y - 0.5 for y in range(1, n_objects)], minor=True)
    ...
    def export_all_statistics(self, base_path: str = None) -> Dict:
        ...
        matrix_path = self.export_dependency_matrix_plot(
            output_path=os.path.join(base_path, f"dependency_matrix_{timestamp}.png")
        )
        ...
        export_info = {
            ...
            'dependency_matrix_png': matrix_path,
        ...
        print(f"Dependency Matrix PNG: {os.path.basename(matrix_path)}")
...
if __name__ == "__main__":
    ...
    if grapher.connect_to_database(connection_string):
        ...
        if grapher.analyze_database_schema():
            ...
            # Generate dependency matrix plot
            try:
                print("\nGenerating dependency matrix plot...")
                matrix_path = grapher.export_dependency_matrix_plot()
                print(f"Dependency matrix plot saved to: {matrix_path}")
                
                # Also generate interactive Plotly version
                matrix_fig = grapher.create_dependency_matrix_plot()
                matrix_fig.write_html("dependency_matrix_interactive.html")
                print("Interactive dependency matrix saved to: dependency_matrix_interactive.html")
                
            except Exception as e:
                print(f"Error generating dependency matrix plot: {e}")
