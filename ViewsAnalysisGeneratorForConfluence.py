...
def get_available_schemas(views):
    """Get list of all available schemas from the views data"""
    schemas = set()
    for view in views:
        schema = view['view_info']['schema']
        schemas.add(schema)
...
def create_safe_filename(schema_name, view_name):
    """Create a safe filename from schema and view names"""
    # Remove or replace characters that are problematic in filenames
    safe_schema = re.sub(r'[<>:"/\\|?*]', '_', schema_name)
    safe_view = re.sub(r'[<>:"/\\|?*]', '_', view_name)
    return f"{safe_schema} - {safe_view}"
...
def create_table_metadata(table):
    ...
    # Basic metadata
    metadata = {
        "title": f"{schema_name} - {view_name}",
        "view_name": view_name,
        "schema": schema_name,
        "type": "View",
        "complexity": complexity,
        "generated_date": datetime.now().isoformat(),
        "description": f"Analysis and documentation for table {view_name} in schema {schema_name}"
    }
    
    # Add table statistics if available
    if view_info.get('row_count'):
        metadata['row_count'] = view_info['row_count']
    
    if view_info.get('size_mb'):
        metadata['size_mb'] = view_info['size_mb']
...
def create_content_properties_adf(schema_name, view_name, complexity, column_count=None, row_count=None):
    """
    Create ADF content for page properties section using Confluence content-properties extension
    
    Args:
        schema_name (str): Name of the schema
        view_name (str): Name of the table
...
def generate_table_page(table):
    ...
    row_count = view_info.get('row_count')
    properties_adf = create_content_properties_adf(schema_name, view_name, complexity, column_count, row_count)
...
def generate_table_confluence_files(json_file_path, output_dir="./confluence_docs/views", selected_schemas=None):
    """Generate separate Confluence ADF files and metadata for each table"""

    # Load JSON data
    views = load_json_data(json_file_path)
    if not views:
    ...
    # Filter views by selected schemas if specified
    if selected_schemas:
        filtered_views = []
        for view in views:
            schema = view['view_info']['schema']
    ...
    for view in views:
        ...
        adf_content = generate_table_page(view)

        # Create metadata
        metadata = create_table_metadata(view)

        # Create filename base - keeping original capitalization
        filename_base = create_safe_filename(schema_name, view_name)
        ...
        # Count views per schema for summary
    ...
    # Print summary
    print(f"\nSuccessfully generated {len(generated_files)} files ({len(generated_files)//2} views):")
    print("\nviews by schema:")
...
def parse_command_line_args():
    ...
    parser = argparse.ArgumentParser(description='Generate individual Confluence pages and metadata for views')
    parser.add_argument('--input', '-i', default='./export/views_analysis_all_schemas.json',
                        help='Input JSON file path (default: ./export/views_analysis_all_schemas.json)')
    parser.add_argument('--output', '-o', default='./confluence_docs/views',
                        help='Output directory (default: ./confluence_docs/views)')
...
def main():
    ...
    views = load_json_data(json_file)
    if not views:
        print("Failed to load JSON data")
        return

    available_schemas = get_available_schemas(views)
